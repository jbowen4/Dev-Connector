...
router.get('/', async (req, res) => {
    try {
        const profiles = await Profile.find().populate('user', ['name', 'avatar']);  // .populate adds the user, user name, and user avatar based on the user id saved w/ the profile
        res.json(profiles);
    } catch(err) {
        console.error(err.message);
        res.status(500).send('Server error');
...
router.put('/experience', [ auth, [
    ...
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.erray() });
    }
    ...
    try {
        ...
        profile.experience.unshift(newExp); // unshift() adds new element to the BEGINNING of experience array
        await profile.save();
        res.json(profile);
    } catch(err) {
        console.error(err.message);
